...
<title>Upload Recipe Login Check Page</title>
</head>
<body>
...
ResultSet rs = st.executeQuery("select * from user where user_name ='"+username+"'");
 ...
 if(rs.next()){
	if(rs.getString(6).equals(password)){
		...
		RequestDispatcher rd = request.getRequestDispatcher("uploadRecipe.jsp");
		rd.include(request, response);
	}
	else{
		response.sendRedirect("uploadRecipeLogin.jsp");
	}
